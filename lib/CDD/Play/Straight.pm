...
sub _validate {
    ...
    if ( @{$cards} == 5) {
        if ($cards->[0]->val != $cards->[1]->val and
            ...
            $CDD::Card::RANK_VAL{$cards->[0]->rank} == $CDD::Card::RANK_VAL{$cards->[1]->rank}+1 and
            ...
            $CDD::Card::RANK_VAL{$cards->[3]->rank} == $CDD::Card::RANK_VAL{$cards->[4]->rank}+1 and
            not all {$_->suit eq $cards->[0]->suit} @{$cards}
           ) {
            return $cards;
        }
    }
