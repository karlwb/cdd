...
sub is_valid_group_of_cards {
    my $grpid = undef;
    eval {$grpid = find_grpid_for_cards(@_);};
...
sub find_number_of_available_groups_for_size {
    ...
    my $q =
"select count(*) from grpavail, grp where grpavail.grp_id = grp.id and and grpavail.available=1 and grp.numcards = $size";
    my $sth = $dbh->prepare($q);
    $sth->execute();
    my @row = $sth->fetchrow_array();
    my $ret = $row[0];
...
sub find_number_of_available_groups_for_size_excluding_cards {
    ...
    my $q =
"select count(*) from grpavail, grp where grpavail.grp_id = grp.id and grpavail.available=1 and grp.numcards = $size and grp.id not in (select distinct grp_id from grpcard where card_id in ($cards))";
...
sub find_number_of_available_groups_with_equal_value {
    my ($size, $value) = @_;
    my $q =
"select count(*) from grpavail, grp where grpavail.grp_id = grp.id and and grpavail.available=1 and grp.numcards = $size and grp.value = $value";
    my $sth = $dbh->prepare($q);
    $sth->execute();
    my @row = $sth->fetchrow_array();
    my $ret = $row[0];
...
sub find_number_of_available_groups_with_equal_value_excluding_cards {
    ...
    my $cards = join(", ", map {"'$_'"} @cards);
    my $q =
"select count(*) from grpavail, grp where grpavail.grp_id = grp.id and and grpavail.available=1 and grp.numcards = $size and grp.value = $value and grp.id not in (select distinct grp_id from grpcard where card_id in ($cards))";
    my $sth = $dbh->prepare($q);
    $sth->execute();
    my @row = $sth->fetchrow_array();
    my $ret = $row[0];
